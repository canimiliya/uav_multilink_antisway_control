"""Full-3D outer-loop controller interface.

The contract is deliberately independent from the legacy ``SwayController``
...
class AccelerationCommand3D:
    ...
    def __post_init__(self) -> None:
        ...
        if values.shape != (3,) or not np.isfinite(values).all():
            raise ValueError("3-D acceleration command must be a finite 3-vector")
    ...
    @classmethod
    def from_array(cls, value: Sequence[float] | np.ndarray) -> "AccelerationCommand3D":
        array = np.asarray(value, dtype=float)
        if array.shape != (3,):
            raise ValueError("3-D command shape must be (3,)")
        return cls(*array.tolist())


class OuterLoopController(Protocol):
    """Protocol required of every three-axis outer-loop controller."""

    def command(self, state: object, reference: object, dt: float) -> np.ndarray: ...


@dataclass
class AccelerationLimiter3D:
    ...
    def __post_init__(self) -> None:
        if self.absolute_limit_m_s2 <= 0.0 or self.slew_limit_m_s2_per_update < 0.0:
            raise ValueError("invalid 3-D command limits")
        ...
        if not np.isfinite(self.previous).all():
            raise ValueError("previous 3-D command must be finite")

    def reset(self, value: Sequence[float] | np.ndarray = (0.0, 0.0, 0.0)) -> None:
        value = np.asarray(value, dtype=float)
        if value.shape != (3,) or not np.isfinite(value).all():
            raise ValueError("3-D limiter reset value must be a finite 3-vector")
        self.previous = value.copy()

    def limit(self, command: Sequence[float] | np.ndarray) -> AccelerationCommand3D:
        ...
        return AccelerationCommand3D.from_array(self.previous)
